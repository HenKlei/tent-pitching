...
class DiscontinuousGalerkin:
    ...
    def _get_transformed_solution(self, tent, local_solution, t_ref):
    ...
    def _get_rhs_vector(self, tent, local_solution, t_ref):
        transformed_solution = self._get_transformed_solution(tent, local_solution, t_ref)

        vector = np.zeros(sum([int(1. / self.local_space_grid_size)
                               for element in tent.get_space_patch().get_elements()]))
        pos = 0

        for j, function in enumerate(transformed_solution):
            ...
            for i, function_value_central in enumerate(function_value):
                ...
                if pos == 0:
                    ...
                    if i == 0:
                        # Does this work in every case???? -> Order of transformed_solutions????
                        function_value_left = transformed_solution[j-1].get_values()[-1]
                        function_value_right = function_value[i+1]
                    elif i == int(1. / self.local_space_grid_size) - 1:
                        # Does this work in every case???? -> Order of transformed_solutions????
                        function_value_right = transformed_solution[j+1].get_values()[0]
                        function_value_left = function_value[i-1]
                    else:
                        function_value_left = function_value[i-1]
                        function_value_right = function_value[i+1]

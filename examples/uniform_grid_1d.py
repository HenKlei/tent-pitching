...
#    plot_space_function(u_0, title='Initial condition interpolated to DG space')
    ...
    plot_space_time_function(u, inverse_transformation, title=f'Space time solution for mu={MU}',
...
#                           title=f'Local solution on reference tent for mu={MU}', three_d=True)
    ...
    with open(f'u_linear_transport_mu_{str(MU).replace(".", "_")}', 'wb') as file_obj:
